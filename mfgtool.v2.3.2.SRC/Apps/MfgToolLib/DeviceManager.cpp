		...
		case DEVICE_REMOVAL_EVT:
		{
			...
			/*	case DEV_MX23:		//Hid
				...
				if(nsInfo.Device)
				{
					...
					//delete device
					std::list<Device*>::iterator deviceIt;
					for(deviceIt=g_devClasses[DeviceClass::DeviceTypeMxHid]->_devices.begin(); deviceIt!=g_devClasses[DeviceClass::DeviceTypeMxHid]->_devices.end(); ++deviceIt)
					{
					...
					WaitForSingleObject(g_devClasses[DeviceClass::DeviceTypeMxHid]->devicesMutex, INFINITE);
					delete (*deviceIt);
					g_devClasses[DeviceClass::DeviceTypeMxHid]->_devices.erase(deviceIt);
					...
					ReleaseMutex(g_devClasses[DeviceClass::DeviceTypeMxHid]->devicesMutex);
				}
			}
			break;
